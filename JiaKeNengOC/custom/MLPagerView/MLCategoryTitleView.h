//
//  MLCategoryTitleView.h
//  JiuXiaoer
//
//  Created by lele on 2021/9/10.
//  Copyright © 2021 lele. All rights reserved.
//
...
@class MLCategoryTitleView;

@protocol MLCategoryTitleViewDataSource <NSObject>
@optional
// 如果将JXCategoryTitleView嵌套进UITableView的cell，每次重用的时候，JXCategoryTitleView进行reloadData时，会重新计算所有的title宽度。所以该应用场景，需要UITableView的cellModel缓存titles的文字宽度，再通过该代理方法返回给JXCategoryTitleView。
// 如果实现了该方法就以该方法返回的宽度为准，不触发内部默认的文字宽度计算。
- (CGFloat)categoryTitleView:(MLCategoryTitleView *)titleView widthForTitle:(NSString *)title;
@end


@interface MLCategoryTitleView : JXCategoryIndicatorView

@property (nonatomic, weak) id<MLCategoryTitleViewDataSource> titleDataSource;

@property (nonatomic, strong) NSArray <NSString *>*titles;

@property (nonatomic, assign) NSInteger titleNumberOfLines; //默认：1

@property (nonatomic, strong) UIColor *titleColor;      //默认：BlackColor

@property (nonatomic, strong) NSArray *titleColors;  //多颜色

@property (nonatomic, strong) UIColor *titleSelectedColor;      //默认：[UIColor redColor]

@property (nonatomic, strong) NSArray *titleSelectedColors;
