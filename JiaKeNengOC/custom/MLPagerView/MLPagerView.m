...
@class MLPagerListContainerScrollView;
@class MLPagerListContainerCollectionView;

@interface MLPagerView () <UITableViewDataSource, UITableViewDelegate, MLPagerListContainerViewDelegate>
@property (nonatomic, weak) id<MLPagerViewDelegate> delegate;
@property (nonatomic, strong) JXPagerMainTableView *mainTableView;
@property (nonatomic, strong) MLPagerListContainerView *listContainerView;
@property (nonatomic, strong) UIScrollView *currentScrollingListView;
@property (nonatomic, strong) id<MLPagerViewListViewDelegate> currentList;
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, id<MLPagerViewListViewDelegate>> *validListDict;
@property (nonatomic, strong) UIView *tableHeaderContainerView;
@end

@implementation MLPagerView

- (instancetype)initWithDelegate:(id<MLPagerViewDelegate>)delegate {
    return [self initWithDelegate:delegate listContainerType:MLPagerListContainerType_CollectionView];
}

- (instancetype)initWithDelegate:(id<MLPagerViewDelegate>)delegate listContainerType:(MLPagerListContainerType)type {
    ...
    if (self) {
        ...
        _listContainerView = [[MLPagerListContainerView alloc] initWithType:type delegate:self];
    }
    return self;
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    CGFloat height = MAX(self.bounds.size.height - [self.delegate heightForPinSectionHeaderInPagerView:self] - self.pinSectionHeaderVerticalOffset, 0);
    return height == 0?screenH:height;
}
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) { // 在第一行情况下， 将header调整到最底层。
        [tableView sendSubviewToBack:self.tableHeaderContainerView];
    }
}
...
- (NSInteger)numberOfListsInlistContainerView:(MLPagerListContainerView *)listContainerView {
    return [self.delegate numberOfListsInPagerView:self];
}

- (id<MLPagerViewListViewDelegate>)listContainerView:(MLPagerListContainerView *)listContainerView initListForIndex:(NSInteger)index {
    id<MLPagerViewListViewDelegate> list = self.validListDict[@(index)];
    if (list == nil) {
        list = [self.delegate pagerView:self initListAtIndex:index];
        __weak typeof(self)weakSelf = self;
        __weak typeof(id<MLPagerViewListViewDelegate>) weakList = list;
...
{
    [self.delegate contentScrollViewVillChanged:contentOffset];
}


- (void)listContainerViewWillBeginDragging:(MLPagerListContainerView *)listContainerView {
    self.mainTableView.scrollEnabled = NO;
}

- (void)listContainerViewWDidEndScroll:(MLPagerListContainerView *)listContainerView {
    self.mainTableView.scrollEnabled = YES;
}

- (void)listContainerView:(MLPagerListContainerView *)listContainerView listDidAppearAtIndex:(NSInteger)index {
    self.currentScrollingListView = [self.validListDict[@(index)] listScrollView];
    for (id<MLPagerViewListViewDelegate> listItem in self.validListDict.allValues) {
...
- (Class)scrollViewClassInlistContainerView:(MLPagerListContainerView *)listContainerView {
...
-(void)pagerScrollViewIndex:(NSInteger)index
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(pagerScrollViewSelectIndex:)]) {
        return [self.delegate pagerScrollViewSelectIndex:index];
    }
}

@end

@implementation MLPagerView (UISubclassingGet)
...
@implementation MLPagerView (UISubclassingHooks)
...
- (void)preferredProcessMainTableViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (scrollView.contentOffset.y < self.mainTableViewMaxContentOffsetY) {
        ...
        for (id<MLPagerViewListViewDelegate> list in self.validListDict.allValues) {
