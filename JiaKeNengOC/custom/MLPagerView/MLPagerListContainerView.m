...
@interface MLPagerListContainerScrollView: UIScrollView <UIGestureRecognizerDelegate>
@property (nonatomic, assign, getter=isCategoryNestPagingEnabled) BOOL categoryNestPagingEnabled;
@end
@implementation MLPagerListContainerScrollView
...
@interface MLPagerListContainerCollectionView: UICollectionView <UIGestureRecognizerDelegate>
@property (nonatomic, assign, getter=isCategoryNestPagingEnabled) BOOL categoryNestPagingEnabled;
@end
@implementation MLPagerListContainerCollectionView
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    if (self.isCategoryNestPagingEnabled) {
        if ([gestureRecognizer isMemberOfClass:NSClassFromString(@"UIScrollViewPanGestureRecognizer")]) {
            CGFloat velocityX = [(UIPanGestureRecognizer *)gestureRecognizer velocityInView:gestureRecognizer.view].x;
...
@interface MLPagerListContainerViewController : UIViewController
...
@implementation MLPagerListContainerViewController
- (void)dealloc
{
...
@interface MLPagerListContainerView () <UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource>
@property (nonatomic, weak) id<MLPagerListContainerViewDelegate> delegate;
...
@property (nonatomic, strong) MLPagerListContainerViewController *containerVC;
@end

@implementation MLPagerListContainerView

- (instancetype)initWithType:(MLPagerListContainerType)type delegate:(id<MLPagerListContainerViewDelegate>)delegate{
...
- (void)initializeViews {
    _listCellBackgroundColor = WhiteColor;
    _containerVC = [[MLPagerListContainerViewController alloc] init];
    ...
    if (self.containerType == MLPagerListContainerType_ScrollView) {
        if (self.delegate &&
            ...
            _scrollView = [[MLPagerListContainerScrollView alloc] init];
        }
        ...
        if (self.delegate &&
            ...
            _collectionView = [[MLPagerListContainerCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
        }
...
- (void)layoutSubviews {
    ...
    if (self.containerType == MLPagerListContainerType_ScrollView) {
        if (CGRectEqualToRect(self.scrollView.frame, CGRectZero) ||  !CGSizeEqualToSize(self.scrollView.bounds.size, self.bounds.size)) {
            ...
            [_validListDict enumerateKeysAndObjectsUsingBlock:^(NSNumber * _Nonnull index, id<MLPagerViewListViewDelegate>  _Nonnull list, BOOL * _Nonnull stop) {
...
- (void)setCategoryNestPagingEnabled:(BOOL)categoryNestPagingEnabled {
    ...
    if ([self.scrollView isKindOfClass:[MLPagerListContainerScrollView class]]) {
        ((MLPagerListContainerScrollView *)self.scrollView).categoryNestPagingEnabled = categoryNestPagingEnabled;
    }else if ([self.scrollView isKindOfClass:[MLPagerListContainerCollectionView class]]) {
        ((MLPagerListContainerCollectionView *)self.scrollView).categoryNestPagingEnabled = categoryNestPagingEnabled;
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    id<MLPagerViewListViewDelegate> list = _validListDict[@(indexPath.item)];
    if (list != nil) {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(pagerScrollViewIndex:)]) {
        [self.delegate pagerScrollViewIndex:self.currentIndex];
    }
}
...
//            [self contentOffsetOfContentScrollViewDidChanged:contentOffset];
            [self.delegate contentScrollViewDidChanged:contentOffset];
        }
...
- (void)reloadData {
    for (id<MLPagerViewListViewDelegate> list in _validListDict.allValues) {
...
- (void)initListIfNeededAtIndex:(NSInteger)index {
    ...
    id<MLPagerViewListViewDelegate> list = _validListDict[@(index)];
    if (list != nil) {
        //列表已经创建好了
        return;
    }
...
-(void)selectPagerIndex:(NSInteger)index
{
