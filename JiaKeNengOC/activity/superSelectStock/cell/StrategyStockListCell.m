...
@implementation StrategyStockListCell
...
- (void)createUI {
    ...
    self.rightScrollView.contentSize = CGSizeMake(480, 0);
    ...
    [self.priceCountLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.rightScrollView).offset(195);
    ...
    [self.addPondBtn setTitle:@"-" forState:UIControlStateNormal];
    [self.addPondBtn setTitleColor:[UIColor colorWithHexString:@"#FF303E"] forState:UIControlStateNormal];
    ...
    [self.addPondBtn border:1 borderColor:[UIColor colorWithHexString:@"#FF303E"]];
...
- (void)setModel:(StockPondModel *)model {
    ...
    self.nameLabel.text = isNilString(model.stockChinaName) ? @"-" : model.stockChinaName;
    self.tagLabel.text = isNilString(model.stockCode) ? @"-" : model.stockCode;
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    if (!_isNotification) {
        // 发送通知
        [[NSNotificationCenter defaultCenter] postNotificationName:strategyStockScrollNotification object:self userInfo:@{@"cellOffX":@(scrollView.contentOffset.x)}];
    }
    _isNotification = NO;
}
