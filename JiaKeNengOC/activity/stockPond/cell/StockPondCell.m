...
@interface StockPondCell ()

@end

@implementation StockPondCell
...
- (void)createUI {
    ...
    self.rightScrollView.contentSize = CGSizeMake(510, 0);
    ...
    [self.addAIBtn setTitleColor:[UIColor colorWithHexString:@"#00B656"] forState:UIControlStateNormal];
    ...
    [self.addAIBtn border:1 borderColor:[UIColor colorWithHexString:@"#00B656"]];
    ...
    self.removePondBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.removePondBtn setTitle:@"-" forState:UIControlStateNormal];
    [self.removePondBtn setTitleColor:[UIColor colorWithHexString:@"#FF303E"] forState:UIControlStateNormal];
    self.removePondBtn.titleLabel.font = [UIFont systemFontOfSize:15];
    [self.removePondBtn acs_radiusWithRadius:2 corner:UIRectCornerAllCorners];
    [self.removePondBtn border:1 borderColor:[UIColor colorWithHexString:@"#FF303E"]];
    [self.removePondBtn addTarget:self action:@selector(removePondBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    [self.rightScrollView addSubview:self.removePondBtn];
    [self.removePondBtn mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [self.selectBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.rightScrollView).offset(470);
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    if (!_isNotification) {
        // 发送通知
        [[NSNotificationCenter defaultCenter] postNotificationName:stockPondScrollNotification object:self userInfo:@{@"cellOffX":@(scrollView.contentOffset.x)}];
    }
    _isNotification = NO;
}
...
- (void)removePondBtnClick:(UIButton *)sender {
    [self.delegate removePondBtnAction:self];
}
